...
export class ViewTaskDetailsComponent {
  ...
  getTaskById() {
    this.service.getTaskById(this.taskId).subscribe((res) => {
      ...
      console.log("Links Data:", JSON.stringify(this.taskData.links));
      console.log("keepinloousersLhow may user are ther ", this.taskData.keepInLoopUsers);// Check the exact structure

      

    // If taskData.keepInLoopUsers is an array of user objects
    if (this.taskData.keepInLoopUsers && Array.isArray(this.taskData.keepInLoopUsers)) {
      this.taskData.keepInLoopUsers.forEach((user: any) => {
        // Push the user's name to the array
        this.keepInLoopUsernames.push(user.name);
      });
    }
    ...
    this.service.getCommentsByTaskId(this.taskId).subscribe((res) => {
      this.comments = res.map((comment: any) => {
        if (comment.imageName && !comment.imageName.startsWith("http")) {
          ...
          comment.imageName = `${environment.BASE_URL}api/files/comment/images/${comment.imageName}`;
        }
        if (comment.voiceName && !comment.voiceName.startsWith("http")) {
          ...
          comment.voiceName = `${environment.BASE_URL}api/files/comment/voice/${comment.voiceName}`;
        }
        return comment;
      });
    });
